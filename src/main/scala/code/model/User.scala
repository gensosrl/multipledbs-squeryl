...
object User extends User with MetaRecord[User] with ProtoAuthUserMeta[User] with Loggable {
  ...
  def findByEmail(in: String): Box[User] = using(UserSchema.session) {
    UserSchema.users.where(user => user.email === in).headOption
  }

  def findByUsername(in: String): Box[User] = using(UserSchema.session) {
    UserSchema.users.where(user => user.username === in).headOption
  }

  def find(id: Long): Box[User] = using(UserSchema.session) { UserSchema.users.lookup(id) }

  def save(inst: User) = using(UserSchema.session) { UserSchema.users.insertOrUpdate(inst) }
  ...
  */
  ...
  override def findAllByUsername(username: String): List[User] = using(UserSchema.session) { UserSchema.users.where(_.username === username).toList }
  
  override def findAllByEmail(email: String): List[User] = using(UserSchema.session) {UserSchema.users.where(_.email === email).toList}
...
object SystemUser {
  private val username = "lift-squeryl-app"
  private val email = "juan@genso.com.bo"

  lazy val user: User = User.findByUsername(username) openOr {
    User.save(User.createRecord
      .name("Lift Squeryl App")
      .username(username)
      .email(email)
      .locale("en_US")
...
object UserSchema extends AuthUserSchema[User] {
  ...
  def session = Session.create(
          DriverManager.getConnection("jdbc:h2:mem:dbname2;DB_CLOSE_DELAY=-1", "sa", ""),
          new H2Adapter
        )
