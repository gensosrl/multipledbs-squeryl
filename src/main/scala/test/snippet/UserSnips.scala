...
import net.liftmodules.squerylauth.LoginRedirect
import net.liftmodules.squerylauth.model.ExtSession
...
sealed trait UserSnippet extends SnippetHelper with Loggable {
  ...
  def title(xhtml: NodeSeq): NodeSeq = serve { user =>
    <lift:head>
      <title lift="Menu.title">{"Lift Squeryl App: %*% - "+user.username.get}</title>
    </lift:head>
  }
}
